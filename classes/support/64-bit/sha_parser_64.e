note
	description: "[
		A 64-bit implementation of an {SHA_PARSER}.
...
feature {NONE} -- Implementation
	...
	pad_with_one
		...
		local
			...
			n: NATURAL_64
		do
			n := 0x80000000_00000000
				-- Check the last block
			b := blocks.last
			if b.count < {SHA_BLOCK_64}.words_per_block then
	...
	pad
		...
		do
			if blocks.count = 0 then
					-- Handle an empty string input by adding a block that has
					-- a one bit followed by zeros (even the count is zero)
				create b
			n := 0x80000000_00000000
				b.put (n, 0)
					-- Leave the rest of this block set to zeros
				is_one_padded := True
				is_padded := True
			else
				...
				b := blocks.last
				if b.count > {SHA_BLOCK_64}.words_per_block - 2 then
						-- There is no room for length, so fill with zeros and make new block
					from i := b.count
					until i >= {SHA_BLOCK_64}.words_per_block
					loop
						b.put (0, i)
						i := i + 1
					end
				...
				from i := b.count
				until i >= {SHA_BLOCK_64}.words_per_block - 2
				loop
					b.put (0, i)
					i := i + 1
				end
					-- Now add the length vectot.  Remember, a block is zero-based
				b.put (0, {SHA_BLOCK_64}.words_per_block - 2)
				b.put ((byte_count * 8).as_natural_32, {SHA_BLOCK_64}.words_per_block - 1)
				is_padded := True
			end
		end

	i_th_word (a_index: INTEGER_32): NATURAL_64
			-- The `a_index'th word in the `file_pointer' input
		local
			i: INTEGER_32
		do
			i := (a_index) * {SHA_BLOCK_64}.bytes_per_word
--			Result := buffer.read_natural_64 (i)
--			Result := buffer.read_natural_64_le (i)
			Result := buffer.read_natural_64_be (i)
		end

	partial_word: NATURAL_64
		...
		do
			...
			loop
				...
				check
					not_too_many_byte: bc < {SHA_BLOCK_64}.bytes_per_word
			...
			from i := bc
			until i >= {SHA_BLOCK_64}.bytes_per_word
			loop
				Result := Result.bit_shift_left (8)
				i := i + 1
			end
	...
	i_th_block (a_index: INTEGER_32): SHA_BLOCK_64
...
--			block_is_full: Result.count = anchor_block.words_per_block
	...
	partial_block: SHA_BLOCK_64
		...
		do
			create Result
			from i := blocks.count * {SHA_BLOCK_64}.words_per_block
			until i >= word_count
			loop
				w := i_th_word (i)
				Result.put (w, c)
