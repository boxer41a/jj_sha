note
	description: "[
		Shows some simple instructions for using the Utility.\
		]"
...
feature {NONE} -- Implementation / Constants
	...
	Default_title: STRING = "Help Dialog"
			-- Default title for the dialog window.

	Default_message: STRING =
		"SHA Checksum Utility%N%
		%Version 1.0%N%
		%%N%
		%To generate one or more hash digests: %N%
		%   1) enter a string into the %"Input_Message%" field, %N%
		%   2) select one or more of the SHA-xxx check boxes, %N%
		% %N%
		%It is also possible to select a file using the File Open menu or button. %N%
		%%N%
		%An expected hash can be pasted into the Expected Hash field %
		%for comparison with generated hashes.%N%
		 %%N%
		%"
